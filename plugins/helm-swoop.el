...
(defvar helm-swoop-last-window-start nil)

(defun stp-window-scroll-functions (win _start)
  (setq helm-swoop-last-window-start _start)
  ;; (message "SL: start set to %d" helm-swoop-last-window-start)
  )
      ...
      (when (and $key $num)
        (with-selected-window helm-swoop-synchronizing-window
          (progn
            ...
            (setq helm-swoop-last-window-start (window-start))
            ;; (message "ML: start set to %d" helm-swoop-last-window-start)
...
(defun stp-helm-swoop-action ($line)
  (helm-swoop--goto-line
   (when (string-match "^[0-9]+" $line)
     (string-to-number (match-string 0 $line))))
  (set-window-start (selected-window) helm-swoop-last-window-start)
  (let (($regex
         (mapconcat 'identity
...
(defadvice helm-swoop--restore (before helm-swoop--restore-before activate)
  (with-selected-window helm-swoop-synchronizing-window
    (remove-hook 'window-scroll-functions 'stp-window-scroll-functions t))
  )

(defadvice helm-swoop (before helm-swoop-before activate)
  (setq helm-swoop-synchronizing-window (selected-window))
  (with-selected-window helm-swoop-synchronizing-window
    (add-hook 'window-scroll-functions 'stp-window-scroll-functions nil t))
  )
